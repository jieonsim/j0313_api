...
public class Service {
	...
	public void oneYearList(String[][] members) {
		...
		for (int i = 0; i < members.length; i++) {
			...
			if (intOneYear > target.getYear()) {
				System.out.println(members[i][0] + "(" + members[i][1] + ") / 가입후 "
						+ target.until(now, ChronoUnit.MONTHS) + "개월경과");
			}
		}
	}
	...
	public void oneMonthList(String[][] members) {
		...
		try {
			LocalDate oneMonth = now.minusMonths(1); // 한달전의 날짜를 구한다.(LocalDate타입을 Date타입으로 변경하기위해 DateTimeFormatter ->
														// String -> Date타입)
			...
			for (int i = 0; i < members.length; i++) {
				...
				if (date1.getTime() < date2.getTime()) {
					System.out.println(members[i][0] + "(" + members[i][1] + ") / "
							+ (today.getTime() - date2.getTime()) / (1000 * 60 * 60 * 24) + "일 경과");
				}
			}
		...
		try {
			for (int i = 0; i < members.length; i++) {
				...
				if (strToday.substring(0, 10).equals(target.toString().substring(0, 10))) {
					System.out.println(members[i][0] + "(" + members[i][1] + ") / "
							+ (today.getTime() - date1.getTime()) / (1000 * 60 * 60) + "시간 경과");
				}
			}
		} catch (ParseException e) {
			e.printStackTrace();
...
//		String[] joinArray = joinDay.split("-");
//		
//		if(joinArray[1].length() == 1) temp += "-0" + joinArray[1]; // 데이터가 만약 2024-3-5로 왔을 때 월과 일 앞에 -0(하이픈,0)을 붙이기 위함
